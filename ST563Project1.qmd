...
The accuracy of the tuned model is not significantly different than the NIR at the alpha=0.05 confidence level.
...
plot(a)
```
The tree classification model predicts more accurately that the NIR at the alpha=0.05 confidence level.  It also trains relatively quickly compared to the GAM/Smoothing Spline model.
...
I've recoded the same model without searching through sigma in the tuning grid and get not better accuracy than the NIR.
```{r}
svTrain <- predict(svMod, newdata = bankrupt_train)
postResample(svTrain, obs = bankrupt_train$bankrupt)
svConf <-confusionMatrix(svTrain, bankrupt_train$bankrupt)
svConf
```

```{r}
C2 <- exp(seq(-5,-1,len=10))
svTuneGrid2 <- expand.grid(C)
