...
The general additive model is a parametric model that is the linear combination of basis funtions. Below, we will smoothing splines. The smooting splines subject the model loss function minimization to a regularization penalty, lambda. Lambda and degrees of freedom are the tuing parameters for smoothing spline models. Because the model is parametric, it can be used to perform inference on the predictors. The model performs shrinkage of the predictors, but not not select variables. Because of collinearity concerns, however, I did decide to use Principal Components Analysis prior to training the model. The PCA performs variable selection.
